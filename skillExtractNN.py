...
class SkillsExtractorNN:
    ...
    def __init__(self, word_features_dim, dense_features_dim):
        ...
        lstm_emb_phrase = keras.layers.Dense(
            128, activation='relu')(lstm_emb_phrase)

        lstm_emb_cont = keras.layers.LSTM(256)(lstm_input_cont)
        lstm_emb_cont = keras.layers.Dense(
            128, activation='relu')(lstm_emb_cont)
        ...
        x = keras.layers.concatenate(
            [lstm_emb_phrase, lstm_emb_cont, dense_emb])
    ...
    def fit(self, x_lstm_phrase, x_lstm_context, x_dense, y,
        ...
        x_lstm_phrase_seq = keras.preprocessing.sequence.pad_sequences(
            x_lstm_phrase, dtype=np.float32)
        x_lstm_context_seq = keras.preprocessing.sequence.pad_sequences(
            x_lstm_context, dtype=np.float32)
    ...
    def predict(self, x_lstm_phrase, x_lstm_context, x_dense):
        ...
        y = self.model.predict(
            [x_lstm_phrase_seq, x_lstm_context_seq, x_dense])
        return y
